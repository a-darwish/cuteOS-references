...
/*
 * RCS $Revision: 104273 $
 * Checkin $Date: 2006-11-02 17:32:02 +0000 (Thu, 02 Nov 2006) $
...
extern "C" {
  ...
  void *__cxa_get_exception_ptr(UCB *);
  ...
  __cxa_type_match_result __cxa_type_match(UCB *, const std::type_info *,
...
void __cxa_call_unexpected(UCB *ucbp)
{
  ...
  // The base argument is now unused
  ...
  try {
    ...
    for (i = 0; i < rtti_count; i++) {
      void *matched_object;
      const type_info *fnspec = (const type_info *)__ARM_resolve_target2((void *)rtti_offset_array_addr);
 ...
 * objects and then exits its catch by a means other than rethrow, or by us
 * when one of our objects is to be destroyed.
 */

#pragma push
#pragma exceptions_unwind

static void armcpp_exception_cleanup(_Unwind_Reason_Code c, UCB *ucbp)
{
    __cxa_eh_globals *g = __cxa_get_globals();
    __cxa_exception *ep = ucbp_to_ep(ucbp);
    ...
    {
        // The object can be destroyed and freed
    ...
    {
        // handlerCount != 0 means the object was rethrown from
        // within a handler. The fact we have arrived here means a
        // foreign runtime called us, since our own runtime only
        // calls us when handlerCount == 0. So just undo the
        // housekeeping related to the rethrow.
...
void __cxa_throw(void *eop, const type_info *t, cppdtorptr d)
{
  ...
  ucbp->exception_cleanup = armcpp_exception_cleanup;
  ep->exceptionType = t;
 ...
 */
__cxa_type_match_result __cxa_type_match(UCB *ucbp, const type_info *match_type,
...
void *__cxa_get_exception_ptr(UCB *ucbp)
{
...
void __cxa_end_catch(void)
{
  ...
  if (ep->handlerCount == 0) {
    ...
    uint32_t propagationcount = ep->propagationCount;
    ...
    if (propagationcount == 0) {
      // the object is not being rethrown
      _Unwind_DeleteException(ucbp);
    }
